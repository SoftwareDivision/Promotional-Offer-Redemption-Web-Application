...
builder.Services.AddSwaggerGen(options =>
{
    ...
    // Custom schema IDs to resolve naming conflicts
    options.CustomSchemaIds(type =>
    {
        ...
        if (type.IsGenericType)
        {
            ...
            if (genericTypeDefinition.Name == "ApiResponse`1")
            {
                var dataType = genericArguments[0];
                return $"ApiResponse{dataType.Name}";
            }
            
            // Handle other generic types like List<T>
            if (genericTypeDefinition.Name == "List`1")
            {
                return $"List{genericArguments[0].Name}";
            }
        }
        
        // Handle namespace-specific types to avoid conflicts
        if (type.FullName.Contains("Reseller"))
        {
            return $"Reseller{type.Name}";
        }
        
        if (type.FullName.Contains("Manufacturer"))
        {
            return $"Manufacturer{type.Name}";
        }
        
        if (type.FullName.Contains("Shopkeeper"))
        {
            return $"Shopkeeper{type.Name}";
        }
...
{
    // In production, serve Swagger at /swagger
    app.UseSwagger();
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "Promotional Offer Redemption API V1");
