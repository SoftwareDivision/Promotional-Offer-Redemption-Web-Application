...
return Ok(campaignsWithRewardTiers);
        ...
        public async Task<ActionResult<object>> CreateOrder([FromBody] CreateOrderRequest request)
        {
            ...
            try
            {
                ...
                if (request.Items != null && request.Items.Count > 0)
                {
                    var orderCampaign = await _context.Campaigns.Include(c => c.VoucherProducts).FirstOrDefaultAsync(c => c.Id == order.CampaignId);
                    foreach (var item in request.Items)
                    {
                        var eligibleProduct = await _context.CampaignEligibleProducts
                            .Include(ep => ep.CampaignProduct)
                        ...
                        int productIdToSave = eligibleProduct.CampaignProductId;
                        // If voucher_restricted, use ProductId from CampaignVoucherProduct
                        if (orderCampaign != null && orderCampaign.RewardType == "voucher_restricted")
                        {
                            var voucherProduct = orderCampaign.VoucherProducts.FirstOrDefault(vp => vp.IsActive);
